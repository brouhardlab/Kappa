...
public class BSpline extends Curve {
    ...
    protected double squaredDistanceErrorTerm(ArrayList<Point2D> dataPoints, int datapointIndex, int footpointIndex) {
        ...
        if (d < 0) {
            return (d / (d - k)) * (squared(diff.getX() * T.getX() + diff.getY() * T.getY()) + squared(diff.getX() * N.getX() + diff.getY() * N.getY()));
    ...
    public double fittingIteration(ArrayList<Point2D> dataPoints, int[] footpointIndices, ArrayList<Double> weights, int t) {
